...
'''ANIMALS_WORD = ['It can Fly\nDRBI', 'It barks\nDGO', 'OENDYK', 'GFRIEFA', 'GLOILARTA', 'TAC', 'EHSOR', 'OLIN', 'MYOEKN', 'EEB', 'KDUC',
...
'''
ran_num = randrange(0, (len(COMPANY_NAMES)))
jumbled_rand_word = COMPANY_NAMES[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(COMPANY_NAMES)))
        word.configure(text=COMPANY_NAMES[ran_num])
    ...
    def cheak():
        ...
        if user_word == COMPANY_ANSWER[ran_num]:
            points += 5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="Score: " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text=COMPANY_ANSWER[ran_num])
        else:
