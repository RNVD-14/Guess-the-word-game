...
PLACES_WORD = ['YSOMER','GLREONAMA','GLREONABA', ]

PLACES_ANSWER = ['MYSORE','MANGALORE','BANGALORE', ]

ran_num = randrange(0, (len(PLACES_WORD)))
jumbled_rand_word = PLACES_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(PLACES_WORD)))
        word.configure(text=PLACES_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == PLACES_ANSWER[ran_num]:
            points += 5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="Score: " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text=PLACES_ANSWER[ran_num])
        else:
